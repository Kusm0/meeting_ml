"""
TF-IDF based Topic Type Classification model.
4-class classification: Top level, Sub-topics, Functional, other.
"""
...
class TopicTypeClassifierTFIDF(BaseModel):
    """
    TF-IDF + Logistic Regression for Topic Type Classification.
    ...
    def __init__(
        ...
        """
        Initialize Topic Type Classifier.

        Args:
            max_iter: Maximum iterations for solver
            class_weight: Class weight strategy
            C: Regularization strength
            use_random_forest: Whether to use Random Forest instead
            n_estimators: Number of trees for Random Forest
        """
        task_config = settings.tasks["topic_type"]
        super().__init__(
            task="topic_type",
            model_type="tfidf",
    ...
    def train(
        ...
        """
        logger.info(
            f"Training Topic Type Classifier (TF-IDF) on "
    ...
    def load(self, path: Path) -> None:
        ...
        self.C = config["C"]
        self.use_random_forest = config["use_random_forest"]
    ...
    def get_feature_importance(
        self, feature_names: List[str], top_k: int = 10
    ) -> Dict[str, List[tuple]]:
        """
        ...
        Returns:
            Dictionary mapping class names to important features
        """
        if not self.is_trained:
            raise ValueError("Model not trained.")
        ...
        if self.use_random_forest:
            ...
            # Per-class coefficients for Logistic Regression
            for i, class_name in enumerate(self.class_names):
                coef = self.model.coef_[i]
                top_idx = np.argsort(coef)[-top_k:][::-1]
                result[class_name] = [
                    (feature_names[j], float(coef[j]))
                    for j in top_idx
                ]
